...
import { Menu, X, User, ChevronDown, Settings, LogOut, LayoutDashboard, Briefcase, FileText } from 'lucide-react';
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <div className="flex items-center justify-between h-16">
          ...
          >
            <div className="w-6 h-6 flex flex-col justify-center items-center">
              <span className={`bg-gray-600 block transition-all duration-300 ease-out h-0.5 w-6 rounded-sm ${isMenuOpen ? 'rotate-45 translate-y-1' : '-translate-y-0.5'}`} />
              <span className={`bg-gray-600 block transition-all duration-300 ease-out h-0.5 w-6 rounded-sm my-0.5 ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`} />
              <span className={`bg-gray-600 block transition-all duration-300 ease-out h-0.5 w-6 rounded-sm ${isMenuOpen ? '-rotate-45 -translate-y-1' : 'translate-y-0.5'}`} />
            </div>
          </button>
        </div>
      ...
      <div className={`md:hidden transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
        <div className="px-4 pt-2 pb-4 space-y-1 bg-white/95 backdrop-blur-md">
          ...
          <MobileNavLink to="/templates" onClick={() => setIsMenuOpen(false)}>
          ...
          {user && (
            <div className="pt-4 mt-4 border-t border-gray-200">
              ...
              <button
                onClick={logoutUser}
                className="w-full mt-2 px-3 py-2.5 text-left text-sm font-medium text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors flex items-center gap-2"
              >
                <LogOut className="w-4 h-4" />
                Sign Out
              </button>
...
const NavLink = ({ to, children, active, icon }) => (
  <Link
    to={to}
    className={`relative flex items-center px-4 py-2 text-sm font-medium transition-all duration-200 ${
      active 

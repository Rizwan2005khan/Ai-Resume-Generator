...
const ResumeBuilder = () => {
 ...
 const saveResume = async () => {
  try {
    ...
    // Handle image file safely: remove object before sending JSON
    if (
      updatedResumeData.personal_info &&
      typeof updatedResumeData.personal_info.image === "object"
    ) {
      updatedResumeData.personal_info.image = undefined;
    }
    ...
    // Include removeBackground flag if needed
    if (removeBackground) {
    ...
    // Update state safely: preserve arrays if backend response is incomplete
    setResumeData((prev) => ({
      ...prev,
      ...data.updatedResume, // overwrite with backend changes
      personal_info: {
        ...prev.personal_info,
        ...data.updatedResume?.personal_info,
      },
      experience: data.updatedResume?.experience || prev.experience,
      education: data.updatedResume?.education || prev.education,
      project: data.updatedResume?.project || prev.project,
      skills: data.updatedResume?.skills || prev.skills,
      template: data.updatedResume?.template || prev.template,
      accent_color: data.updatedResume?.accent_color || prev.accent_color,
      public: typeof data.updatedResume?.public === "boolean" ? data.updatedResume.public : prev.public,
    }));
              ...
              <div className="flex justify-between items-center mb-6 border-b border-gray-300 py-1">
                ...
                <div className="flex items-center">
                  ...
                  <button
                    onClick={() =>
                      setActiveSectionIndex((prevIndex) =>
                        Math.min(prevIndex + 1, sections.length)
                      )
                    }
                    className={`flex items-center gap-1 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all ${
